defmodule ExArk.Serdes.Binary.Fields.Primitives do
  @moduledoc """
  ...
  @spec read(String.t(), InputStream.t()) :: {:ok, Result.t()} | Binary.deserialization_failure()
  def read(typestr, %InputStream{} = stream) when is_binary(typestr),
    do: read(Utilities.ensure_existing_atom(typestr), stream)

  @spec read(Types.primitive_type(), InputStream.t()) :: {:ok, Result.t()} | Binary.deserialization_failure()
  ...
  def write(type, value, stream) when is_binary(type), do: write(Utilities.ensure_existing_atom(type), value, stream)
